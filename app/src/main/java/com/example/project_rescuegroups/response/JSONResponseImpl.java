...
public class JSONResponseImpl {
    ...
    public List<Animal> handleAnimalResponse(String data) throws IOException {
        List<Animal> result = new ArrayList<>();
        try {
            JSONObject responseObject = (JSONObject) new JSONTokener(data).nextValue();
            JSONArray animals = responseObject.getJSONArray("animals");
            Log.e("lengte","length animals: " + animals.length());
            for (int idx = 0; idx < animals.length(); idx++) {
                ...
                if(name.length()>0 && species.length()>0 && breed.length()>0 && sex.length()>0 && birthdate.length()>0 && jsonArray.length() > 0 &&description.length()>0) {
                    ...
                    String url = object.getString("thumbnailUrl");
                        result.add(new Animal(id, name, species, breed, sex, birthdate, url, description));
